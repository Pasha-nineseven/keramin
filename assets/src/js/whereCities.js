$(document).ready(function() {
    //MAP
	if ($('#map').length>0) {
		ymaps.ready(initializePartnersMap);
	}

	//Partners-city-TOGGLE
	$('body').on('click','.js-partners-city', function(e){
		e.preventDefault();
		...
		cityMap.setCenter([t_lat,t_long]);
		cityMap.setZoom(12,
			{
				smooth: true,
		...
		cityMap.balloon.close();
	});

	//Partners-country-TOGGLE
	$('body').on('click','.js-partners-country', function(e){
		e.preventDefault();
		let t_text = $(this).text();
		let t_lat = $(this).attr('data-lat');
		let t_long = $(this).attr('data-long');
...
//ПАРТНЕРЫ
function initializePartnersMap() {
	let pl;
	//массив городов
	let items = [
		{   id: 1, 
	...
	cityMap.controls.add(new ymaps.control.ZoomControl({options: { position: { right: 20, top: 50 }}}));
	...
	for (i = 0; i < items.length; ++i) {
		...
		pl = new ymaps.Placemark(items[i].center,{
